...
def parse_args():
    ...
    parser.add_argument('--num_layers', default=5, type=int, metavar='N', help='num of residual layers')
...
def main(args):
    ...
    from common.graph_utils import laplacian_from_skeleton
    p_dropout = (None if args.dropout == 0.0 else args.dropout)
    eigenVal, eigenVec = laplacian_from_skeleton(dataset.skeleton())
    model_pos = MobiusGCN(eigenVal, eigenVec, args.hid_dim, num_layers=args.num_layers, p_dropout=p_dropout).to(device)
...
#     print(ground_truth.dtype)
    ...
    prediction = evaluate(render_loader, model_pos, device)[0]
...
#     print(prediction.dtype)
    ...
    anim_output = {'Estimation': prediction, 'Ground truth': ground_truth}
